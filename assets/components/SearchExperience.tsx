...
export function SearchExperience() {
  ...
  function selectTrip(id?: string) { if (session.user && verifiedUserId() !== session.user.id) return; if (!session.user) guestIntent.current = true; setSelectedTripId(id); openPanel("trips"); }
  ...
  async function runSearch(mode: SpotSearchMode = "nearby", supplied?: SpotsApiRequest) {
    ...
    try {
      ...
      setResponse(result); setSelectedSpotId(result.spots[0]?.id); setInfoOpen(result.mode === "point" && result.spots.length > 0); setPanelOpen(false);
  ...
  return <main className="relative h-[100svh] w-full overflow-hidden bg-ink text-ink">
    ...
    {!panelOpen && !infoOpen && !authOpen && <div className="absolute right-3 top-[4.8rem] z-20 grid gap-2 rounded-2xl bg-white/95 p-2 shadow-glow">{([ ["terrain", "Ανάγλυφο"], ["satellite", "Δορυφόρος"], ["street", "Δρόμοι"] ] as const).map(([layer, label]) => <button key={layer} aria-pressed={baseLayer === layer} className={baseLayer === layer ? "ui-primary" : "ui-secondary"} onClick={() => setBaseLayer(layer)}>{label}</button>)}<button className={depthVisible ? "ui-primary" : "ui-secondary"} aria-pressed={depthVisible} onClick={() => setDepthVisible(!depthVisible)}>Βάθος</button>{depthVisible && <div className="w-32 text-center text-[9px] font-bold"><div className="depth-scale-gradient h-2 rounded-full" /><p>0 · 5 · 10 · 15 · 20 · 25μ</p><p>EMODnet ~115μ<br />Όχι για πλοήγηση</p></div>}</div>}

    {response && !panelOpen && !infoOpen && !authOpen && <section className="results-panel absolute bottom-3 left-3 right-3 z-20 max-h-[55svh] overflow-y-auto rounded-3xl border border-white/80 bg-white/95 p-3 shadow-glow sm:bottom-4 sm:left-4 sm:right-auto sm:w-[34rem]" aria-label="Αποτελέσματα αναζήτησης"><p className="ui-eyebrow">{response.intent.techniqueLabel} · {response.spots.length}/{response.resultLimit} σημεία</p><h2 className="mb-3 mt-1 text-lg font-black">{response.location.displayName}</h2><ResultNotices response={response} snapshot={snapshot} />
