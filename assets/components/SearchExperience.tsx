...
export function SearchExperience() {
  ...
  return <main className="relative h-[100svh] w-full overflow-hidden bg-ink text-ink">
    ...
    {pickedPoint && !selectedTripId && !response && !panelOpen && !authOpen && !loading && <section className="workflow-panel selected-point" aria-label="Επιλεγμένο σημείο"><div className="flex items-start justify-between gap-2"><div><h2 className="font-black">Επιλεγμένο σημείο</h2><p className="ui-help">{pickedPoint.lat.toFixed(5)}, {pickedPoint.lon.toFixed(5)} · {TECHNIQUE_PROFILES[technique].label}</p></div><button className="ui-close" aria-label="Αφαίρεση pin" onClick={() => editLocation("Γύθειο")}>x</button></div><div className="mt-3 flex flex-wrap gap-2"><button className="ui-primary" onClick={() => void runSearch("point")}>Ανάλυση εδώ</button><button className="ui-secondary" onClick={() => void runSearch("nearby")}>Σημεία κοντά</button><button className="ui-secondary" onClick={() => newTrip(currentDestination())}>Εξόρμηση εδώ</button><button className="ui-secondary" onClick={() => saveCurrentPlace(currentDestination())}>Αποθήκευση σημείου</button><button className="ui-secondary" onClick={() => openPanel("search")}>Ρυθμίσεις αναζήτησης</button></div></section>}
    ...
    {response && !panelOpen && !infoOpen && !authOpen && <section className={`results-panel sheet-${sheetSize} absolute bottom-3 left-3 right-3 z-20 overflow-y-auto rounded-3xl border border-white/80 bg-white/95 p-3 shadow-glow`} aria-label="Αποτελέσματα αναζήτησης"><p className="ui-eyebrow">{response.intent.techniqueLabel} · {response.spots.length}/{response.resultLimit} σημεία</p><h2 className="mb-3 mt-1 text-lg font-black">{response.location.displayName.split(",")[0]}</h2>{response.location.displayName.includes(",") && <details className="ui-help mb-3"><summary className="cursor-pointer py-1 font-bold">Πλήρης περιοχή</summary><p>{response.location.displayName}</p></details>}{!selectedSpot && <ResultNotices response={response} snapshot={snapshot} />}
      {selectedSpot && <div className="mt-3 space-y-2"><h3 className="text-lg font-black">#{selectedSpot.rank} {selectedSpot.name}</h3><ResultNotices response={response} spot={selectedSpot} snapshot={snapshot} /><p className="ui-help">{selectedSpot.techniqueDepthRange.label} · {selectedSpot.seabedLabel}</p><div className="flex flex-wrap gap-2"><button className="ui-primary" onClick={() => setInfoOpen(true)}>Πλήρης ανάλυση</button><button className="ui-secondary" onClick={() => newTrip(currentDestination(selectedSpot))}>Νέα εξόρμηση</button><button className="ui-secondary" onClick={() => saveCurrentPlace(currentDestination(selectedSpot))}>Αποθήκευση σημείου</button></div></div>}
