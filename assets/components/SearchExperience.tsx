...
import { AnalysisDataDetails, ResultNotices, SpeciesProfile, SpotAnalysis } from "./SpotAnalysis";
...
import { preferredTechnique, rememberTechnique, resolveSelectedTrip } from "@/lib/client/tripDraft";

export function SearchExperience() {
  const session = useSession();
  const [technique, setTechnique] = useState<TechniqueId>(() => preferredTechnique("surfcasting"));
  ...
  async function runSearch(mode: SpotSearchMode = "nearby", supplied?: SpotsApiRequest) {
    ...
    try {
      ...
      setMessage(`${result.cache.hit ? "Απάντηση από cache παρόχων" : "Νέα ανάλυση"}. ${result.spots.length} σημεία.${request.saveHistory && session.user ? result.scanId ? " Αποθηκεύτηκε στο ιστορικό." : " Η αποθήκευση ιστορικού δεν επιβεβαιώθηκε." : " Δεν ζητήθηκε αποθήκευση ιστορικού."}`);
  ...
  function pickPoint(point: Coordinates, accuracy?: number, label = pointLabel(point)) {
    if (!session.user) guestIntent.current = true;
    searchSequence.current += 1; setLoading(false); setPickedPoint(point); setGpsAccuracyM(accuracy); setLocation(label); setResponse(undefined); setSnapshot(false); setSelectedSpotId(undefined); setSelectedTripId(undefined); setTripFocus(undefined); setMessage(undefined); setError(undefined); setInfoOpen(false); setAuthOpen(false); setPanelOpen(false);
  }
  ...
  const mapTools = <>
    <p className="ui-label">Χάρτης</p><div className="grid grid-cols-3 gap-1">{([ ["terrain", "Ανάγλυφο"], ["satellite", "Δορυφόρος"], ["street", "Δρόμοι"] ] as const).map(([layer, label]) => <button key={layer} aria-pressed={baseLayer === layer} className={baseLayer === layer ? "ui-primary" : "ui-secondary"} onClick={() => setBaseLayer(layer)}>{label}</button>)}</div>
    <button className={depthVisible ? "ui-primary" : "ui-secondary"} aria-pressed={depthVisible} onClick={() => setDepthVisible(!depthVisible)}>Βαθυμετρία {depthVisible ? "ενεργή" : "ανενεργή"}</button>
    {depthVisible && <div className="ui-help"><div className="depth-scale-gradient h-3 rounded-full" /><p>Βάθος: 0 · 5 · 10 · 15 · 20 · 25 μ</p><p>EMODnet: εκτίμηση βάθους, κελιά ~115 μ. Όχι για πλοήγηση ή ακριβή βολή.</p></div>}
    {(pickedPoint || selectedSpot || response?.location) && <button className="ui-secondary" onClick={() => setRecenterKey((key) => key + 1)}>Στο σημείο</button>}
  </>;

  return <main className="relative h-[100svh] w-full overflow-hidden bg-ink text-ink">
    <FishingMap key={session.user?.id ?? "guest"} userId={session.user?.id} selectedTripId={selectedTripId} tripFocus={tripFocus} location={response?.location} radiusKm={response?.intent.radiusKm ?? radiusKm} spots={response?.spots ?? []} selectedSpotId={selectedSpot?.id} onSelectSpot={(id) => { setSelectedSpotId(id); setInfoOpen(true); setSheetSize("half"); setPanelOpen(false); setAuthOpen(false); }} loading={loading || gpsLoading} baseLayer={baseLayer} depthVisible={depthVisible} recenterKey={recenterKey} tools={mapTools} toolsOpen={layersOpen} toolsVisible={!authOpen && !(panelOpen && panel === "trips")} onToolsToggle={() => setLayersOpen(!layersOpen)} onMeasureStart={() => { setPanelOpen(false); setSheetSize("collapsed"); setLayersOpen(false); }} pickedPoint={pickedPoint} mode={response?.mode} pointAnalysis={response?.pointAnalysis} trips={activeTrip && !trips.some((trip) => trip.id === activeTrip.id) ? [...trips, activeTrip] : trips} onSelectTrip={selectTrip} onPickPoint={pickPoint} />
    <div className="pointer-events-none absolute inset-x-0 top-0 z-30 p-3 sm:p-4"><nav aria-label="Κύρια πλοήγηση" className="workflow-toolbar pointer-events-auto mx-auto flex max-w-[1560px] items-center gap-2 pb-1">
      ...
      <button className="ui-secondary px-3 py-3 shadow-glow" onClick={() => selectTrip(undefined)}>Εξορμήσεις</button><button className="ui-secondary px-3 py-3 shadow-glow" onClick={() => openPanel("library")}>Βιβλιοθήκη</button>
    ...
    {activeTrip && session.user && !authOpen && !(panelOpen && panel === "trips" && selectedTripId === activeTrip.id) && <section className="active-trip-strip" aria-label="Ψαρεύουμε τώρα">
      <div className="min-w-0"><p className="text-xs font-bold text-lagoon"><span className="trip-live-dot" />Ψαρεύουμε τώρα · {activeTrip.fishRecords.reduce((sum, fish) => sum + fish.count, 0)} ψάρια</p><h2 className="truncate font-bold">{activeTrip.locationName}</h2></div>
      <button className="ui-primary" onClick={() => selectTrip(activeTrip.id)}>Συνέχεια ενεργής</button>
    </section>}
    ...
    {panelOpen && panel === "search" && !authOpen && <section className="workflow-panel search-panel" aria-label="Αναζήτηση ψαρότοπων"><div className="flex items-start justify-between gap-3"><div><p className="ui-eyebrow">Ρυθμίσεις χάρτη</p><h1 className="mt-1 text-xl font-black">Αναζήτηση</h1></div><button className="ui-close" aria-label="Κλείσιμο αναζήτησης" onClick={() => setPanelOpen(false)}>x</button></div>
      <form className="mt-4 space-y-3" onSubmit={(event: FormEvent) => { event.preventDefault(); void runSearch("nearby"); }}>
        <label className="ui-label">Τύπος ψαρέματος<select className="ui-input" value={technique} onChange={(event) => { const next = event.target.value as TechniqueId; setTechnique(next); rememberTechnique(next); }}>{Object.values(TECHNIQUE_PROFILES).map((profile) => <option key={profile.id} value={profile.id}>{profile.label}</option>)}</select></label>
        ...
        {session.user ? <label className="ui-check"><input type="checkbox" checked={saveHistory} onChange={(event) => setSaveHistory(event.target.checked)} />Αποθήκευση στο ιστορικό μου</label> : <p className="ui-help">Δεν αποθηκεύεται προσωπικό ιστορικό ως επισκέπτης.</p>}
    ...
    {pickedPoint && !selectedTripId && !response && !panelOpen && !authOpen && !loading && <section className="workflow-panel selected-point" aria-label="Επιλεγμένο σημείο"><div className="flex items-start justify-between gap-2"><div><h2 className="font-black">Επιλεγμένο σημείο</h2><p className="ui-help">{pickedPoint.lat.toFixed(5)}, {pickedPoint.lon.toFixed(5)} · {TECHNIQUE_PROFILES[technique].label}</p></div><button className="ui-close" aria-label="Αφαίρεση pin" onClick={() => editLocation("Γύθειο")}>x</button></div><div className="mt-3 flex flex-wrap gap-2"><button className="ui-primary" onClick={() => newTrip(currentDestination())}>Ψάρεμα εδώ</button><button className="ui-secondary" onClick={() => void runSearch("point")}>Ανάλυση εδώ</button></div><details className="trip-disclosure mt-3"><summary>Περισσότερα για το σημείο</summary><div className="flex flex-wrap gap-2"><button className="ui-secondary" onClick={() => void runSearch("nearby")}>Σημεία κοντά</button><button className="ui-secondary" onClick={() => saveCurrentPlace(currentDestination())}>Αποθήκευση σημείου</button><button className="ui-secondary" onClick={() => openPanel("search")}>Ρυθμίσεις αναζήτησης</button></div></details></section>}
    ...
    {response && !panelOpen && !infoOpen && !authOpen && <section className={`results-panel sheet-${sheetSize} absolute bottom-3 left-3 right-3 z-20 overflow-y-auto rounded-3xl border border-white/80 bg-white/95 p-3 shadow-glow`} aria-label="Αποτελέσματα αναζήτησης"><p className="ui-eyebrow">{response.intent.techniqueLabel} · {response.spots.length}/{response.resultLimit} σημεία</p><h2 className="mb-3 mt-1 text-lg font-black">{response.location.displayName.split(",")[0]}</h2>{response.location.displayName.includes(",") && <details className="ui-help mb-3"><summary className="cursor-pointer py-1 font-bold">Πλήρης περιοχή</summary><p>{response.location.displayName}</p></details>}{!selectedSpot && <ResultNotices response={response} snapshot={snapshot} />}
      {selectedSpot && <div className="mt-3 space-y-2"><h3 className="text-lg font-black">#{selectedSpot.rank} {selectedSpot.name}</h3><ResultNotices response={response} spot={selectedSpot} snapshot={snapshot} /><p className="ui-help">{selectedSpot.techniqueDepthRange.label} · {selectedSpot.seabedLabel}</p><div className="flex flex-wrap gap-2"><button className="ui-primary" onClick={() => newTrip(currentDestination(selectedSpot))}>Ψάρεμα εδώ</button><button className="ui-secondary" onClick={() => setInfoOpen(true)}>Πλήρης ανάλυση</button><button className="ui-secondary" onClick={() => saveCurrentPlace(currentDestination(selectedSpot))}>Αποθήκευση σημείου</button></div></div>}
      <div className="mt-3"><AnalysisDataDetails response={response} spot={selectedSpot} /></div>
      {response.spots.length > 1 && <div className="mt-3 flex gap-2 overflow-x-auto pb-2">{response.spots.map((spot) => <button key={spot.id} className={`w-44 shrink-0 rounded-2xl border p-3 text-left ${spot.id === selectedSpot?.id ? "border-lagoon bg-lagoon text-white" : "border-slate-200 bg-white"}`} onClick={() => setSelectedSpotId(spot.id)}><span className="block text-xs font-black">#{spot.rank} {spot.name}</span><span className="mt-1 block text-[10px] font-semibold">{spot.distanceKm.toFixed(1)}χλμ από το κέντρο</span></button>)}</div>}
