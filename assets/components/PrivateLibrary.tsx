...
export function PrivateLibrary({ user, destination, onTrip, onRecheck, onScan, onSearch, onLogin, onClose }: {
  ...
  return <section className="workflow-panel" aria-label="Η βιβλιοθήκη μου">
    <div className="flex items-start justify-between gap-3"><div><h2 className="text-xl font-black">Η βιβλιοθήκη μου</h2></div><button className="ui-close" aria-label="Κλείσιμο βιβλιοθήκης" onClick={onClose}>x</button></div>
...
function SavePlaceForm({ destination, busy, onSave }: { destination: TripDestination; busy: boolean; onSave: (name: string, notes: string) => void }) {
  ...
  return <form className="space-y-3 rounded-2xl border border-tide/25 bg-tide/10 p-3" onSubmit={(event: FormEvent) => { event.preventDefault(); if (name.trim()) onSave(name.trim(), notes); }}><h3 className="font-black">Αποθήκευση επιλεγμένου σημείου</h3><p className="ui-help">{destination.lat.toFixed(5)}, {destination.lon.toFixed(5)} · {TECHNIQUE_PROFILES[destination.technique].label}. Δεν αποθηκεύεται πρόγνωση ως τρέχουσα.</p><label className="ui-label">Όνομα<input className="ui-input" value={name} required onChange={(e) => setName(e.target.value)} /></label><label className="ui-label">Σημειώσεις<textarea className="ui-input" rows={2} value={notes} onChange={(e) => setNotes(e.target.value)} /></label><button className="ui-primary w-full" disabled={busy}>Αποθήκευση σημείου</button></form>;
}
